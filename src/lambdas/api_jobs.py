...
where multi-value):
    ...
    sort_by        score | qol | comp | newest | oldest
                   | company_asc | company_desc          (default: score)
...
# Hardening: cap user-supplied free-text notes. Notes are stored
# verbatim on the job row; an unbounded body could bloat the item (DynamoDB
# caps at 400KB/item) or be used to stuff junk. 10K chars (~1,500 words) is
# far more than any real note + leaves ample headroom under the item limit.
_MAX_NOTES_CHARS = 10_000


def _notes_ok(notes) -> bool:
    """True if notes is absent or a string within the size cap."""
    return notes is None or (isinstance(notes, str) and len(notes) <= _MAX_NOTES_CHARS)
...
def _browse(event) -> dict:
    ...
    # and rank score within each group. Direction lives in the sort key
    # itself (like newest/oldest), so sort_dir is ignored for these two.
...
def _taxonomy(event) -> dict:
    ...
    return _json(200, {
        ...
        "sort_options": [
            ...
            # group by company, then rank score within each
            # company. Useful for scanning every active role at a target
            # employer side-by-side instead of scattered across the feed.
