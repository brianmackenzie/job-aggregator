...
# ===========================================================================
# hardening: cap user-supplied notes (action + bulk_action).
# ===========================================================================

def test_action_notes_too_long_rejected(aws):
    """A notes blob over the cap is rejected 400 and the row is untouched."""
    db.put_job(_make_job("remoteok:longnote"))
    resp = handler(
        _event("POST /api/jobs/{job_id}/action", {"job_id": "remoteok:longnote"},
               body=json.dumps({"action": "save", "notes": "x" * 10001})),
        None,
    )
    assert resp["statusCode"] == 400
    assert "too long" in json.loads(resp["body"])["error"]
    # Status must NOT have flipped — the write never happened.
    assert db.get_job("remoteok:longnote")["status"] == "active"


def test_action_notes_at_limit_ok(aws):
    """Exactly at the cap is allowed (boundary)."""
    db.put_job(_make_job("remoteok:oknote"))
    resp = handler(
        _event("POST /api/jobs/{job_id}/action", {"job_id": "remoteok:oknote"},
               body=json.dumps({"action": "save", "notes": "x" * 10000})),
        None,
    )
    assert resp["statusCode"] == 200
    assert db.get_job("remoteok:oknote")["status"] == "saved"


def test_bulk_action_notes_too_long_rejected(aws):
    db.put_job(_make_browse_job("test:bulknote", score=50, industries=["tech"]))
    resp = handler(
        _event("POST /api/jobs/bulk_action",
               body=json.dumps({"action": "skip", "job_ids": ["test:bulknote"],
                                "notes": "y" * 10001})),
        None,
    )
    assert resp["statusCode"] == 400
    assert "too long" in json.loads(resp["body"])["error"]
    # Row untouched.
    assert db.get_job("test:bulknote")["status"] == "active"
