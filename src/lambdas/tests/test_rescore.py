...
# ---------------------------------------------------------------------------
# cost: skip the write when score + track are unchanged.
# ---------------------------------------------------------------------------

# A complete score_combined result whose score (82) + track match the
# _make_job default, so the skip gate fires — but with a DIFFERENT rationale
# so a write (when forced) is detectable by reading the row back.
def _unchanged_result(rationale: str = "REWRITTEN by rescore") -> dict:
    return {
        "score":              82,            # == _make_job default
        "tier":               "T2",
...
def test_unchanged_score_skips_write(aws):
    """A row whose freshly-computed score + track equal what's stored is
    NOT written: updated=0, unchanged_skipped=1, and the existing row is
    left byte-for-byte intact (proven via the untouched rationale)."""
    db.put_job(_make_job("test:noop"))   # score=82, rationale="Previously-good rationale."

    with patch("lambdas.rescore.score_combined",
               return_value=_unchanged_result("DIFFERENT - must NOT be written")):
        result = rescore.handler({}, None)
    ...
    assert result["api_failed_skipped"] == 0
    # The write was skipped, so the stored rationale is the original.
    row = db.get_job("test:noop")
    assert row["semantic_rationale"] == "Previously-good rationale."
    assert row["score"] == 82


def test_changed_score_still_writes(aws):
    ...
    assert db.get_job("test:moved")["score"] == 91


def test_force_write_overrides_skip(aws):
    """force_write=true rewrites even an unchanged row (used to backfill a
    newly-added persisted field). The new rationale lands on the row."""
    db.put_job(_make_job("test:fw"))

    with patch("lambdas.rescore.score_combined",
               return_value=_unchanged_result("FORCED rewrite")):
        result = rescore.handler({"force_write": True}, None)

    assert result["updated"] == 1
    assert result["unchanged_skipped"] == 0
    assert db.get_job("test:fw")["semantic_rationale"] == "FORCED rewrite"


def test_force_semantic_disables_skip(aws):
    """force_semantic implies we paid for fresh Haiku verdicts, so every row
    is written even when the blended score lands on the same integer."""
    db.put_job(_make_job("test:fs"))

    with patch("lambdas.rescore.score_combined",
               return_value=_unchanged_result("FRESH semantic verdict")):
        result = rescore.handler({"force_semantic": True}, None)

    assert result["updated"] == 1
    assert result["unchanged_skipped"] == 0
    assert db.get_job("test:fs")["semantic_rationale"] == "FRESH semantic verdict"


def test_dry_run_never_skips_or_writes(aws):
    """dry_run scores but writes nothing; the skip counter stays 0 because
    the skip gate is for write-avoidance, not a dry-run concept."""
    ...
    assert result["total"] == 1
    assert result["updated"] == 1          # counted as processed, not written
    assert result["unchanged_skipped"] == 0
    # Nothing changed on disk.
    assert db.get_job("test:dry")["semantic_rationale"] == "Previously-good rationale."
