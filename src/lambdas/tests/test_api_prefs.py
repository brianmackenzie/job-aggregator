...
def test_put_rejects_oversized_key(aws):
    ...
    # Nothing stored.
    assert json.loads(handler(_event("GET"), None)["body"])["prefs"] == {}


def test_put_rejects_oversized_value(aws):
    big = ["x" * 1000] * 100   # ~100KB serialized, over the 64KiB cap
    resp = handler(_event("PUT", {"config_key": "hidden_companies", "value": big}), None)
    assert resp["statusCode"] == 400
    assert "too large" in json.loads(resp["body"])["error"]
    assert json.loads(handler(_event("GET"), None)["body"])["prefs"] == {}


def test_put_accepts_value_under_cap(aws):
    ok = ["x" * 100] * 10     # ~1KB, well under cap
    resp = handler(_event("PUT", {"config_key": "hidden_companies", "value": ok}), None)
    assert resp["statusCode"] == 200
    assert json.loads(handler(_event("GET"), None)["body"])["prefs"]["hidden_companies"] == ok
