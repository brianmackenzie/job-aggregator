...
def handler(event, context):
    ...
    for source in list_scrapers:
        try:
            ...
            # Hardening: log the real exception server-side, but
            # return a GENERIC marker to the client. The raw boto/DynamoDB
            # message can carry internal identifiers (ARNs, table names,
            # account id) — don't echo it into the API response.
            log.warn("health_query_failed", source=source, error=str(exc))
            runs.append({
                "source_name": source,
                "status": "error",
                "error_message": "health_query_failed",
            })
