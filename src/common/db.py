...
def update_job_action(
    ...
    Flipping `status` matters for the GSI: status is the ScoreIndex
    partition key, so the row lands in the new partition immediately and
    the dashboard's "active" feed reflects the action on the next read.
    Returns the updated row, or None if the job doesn't exist.

    Cost note: this used to GetItem first purely to check the
    row existed, then UpdateItem with ReturnValues="ALL_NEW" — two Dynamo
    calls per action + a TOCTOU window between them. We now do a single
    conditional UpdateItem (attribute_exists(job_id)); a missing row
    ...
    try:
        resp = table.update_item(
            ...
            # Row must already exist — a missing job_id fails the condition
            # (raises below) instead of silently creating a half-formed row.
...
#   1. Applies the updates sequentially (one update_item per job_id).
#      (A concurrent fan-out via ThreadPoolExecutor is a possible future
#      optimization — boto3 clients are thread-safe — but is not yet wired.)
...
def query_jobs_for_browse(
    ...
    # company_asc / company_desc are a two-pass stable sort so
    # rows cluster by company name, and inside each cluster the highest
    # score floats to the top. Python's sort is stable, so:
    ...
    # that path is a single-pass and can't express the compound key.
