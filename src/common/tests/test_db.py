...
def test_update_job_action_sets_status_and_returns_row(aws):
    """Happy path: existing row gets the new status + notes, and the full
    updated row is returned (ReturnValues=ALL_NEW)."""
    db.put_job(_make_job("remoteok:1"))
    updated = db.update_job_action("remoteok:1", "applied", user_notes="emailed")
    assert updated is not None
    assert updated["status"] == "applied"
    assert updated["user_notes"] == "emailed"
    # Persisted, not just returned.
    assert db.get_job("remoteok:1")["status"] == "applied"


def test_update_job_action_missing_returns_none(aws):
    """A missing job_id returns None — the ConditionExpression on
    attribute_exists fails and is mapped to None."""
    assert db.update_job_action("nope:404", "saved") is None


def test_update_job_action_missing_does_not_create_row(aws):
    """Critical: the conditional write must NOT upsert a phantom row when
    the job_id doesn't exist (a plain UpdateItem would have)."""
    db.update_job_action("ghost:1", "archived")
    assert db.get_job("ghost:1") is None


def test_update_job_action_without_notes_leaves_notes_untouched(aws):
    """Calling without user_notes only flips status; an existing note is
    preserved (the update expression simply doesn't reference it)."""
    job = _make_job("remoteok:9")
    job["user_notes"] = "keep me"
    db.put_job(job)
    db.update_job_action("remoteok:9", "saved")   # no notes arg
    row = db.get_job("remoteok:9")
