...
def test_request_shape_passed_to_anthropic(monkeypatch):
    ...
    out = semantic.semantic_score(SAMPLE_JOB)
    assert out is not None
    # 2026-06-02: system is now a content-block list with prompt caching
    # (cache_control: ephemeral) so the ~5.2K-token system prompt is cached
    # across a rescore batch. The prompt text must still be exactly the profile.
    assert isinstance(captured["system"], list)
    assert captured["system"][0]["text"] == "OWNER_PROFILE"
