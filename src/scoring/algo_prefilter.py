...
# wrong-function exec titles (Finance Director, VP Sales, Public Policy, etc.)
# that must kill BEFORE the generic leadership whitelist can rescue them.
_NEVER_RESCUE_RE = re.compile(
    r"\b(?:"
    + "|".join(
        re.escape(p)
        for p in sorted(CP.WRONG_FUNCTION_EXEC_NEVER_RESCUE, key=len, reverse=True)
    )
    + r")\b",
    re.IGNORECASE,
)

# word-boundary regex for the POSITIVE relevance gate.
# A title with NO match here (and no known industry / dream co / leadership
# title) is skipped pre-Haiku — it carries no signal worth a paid call.
_RELEVANCE_NOUN_RE = re.compile(
    r"\b(?:"
    + "|".join(
        re.escape(n)
        for n in sorted(CP.TITLE_RELEVANCE_NOUNS, key=len, reverse=True)
    )
    + r")\b",
    re.IGNORECASE,
)
...
def _check_wrong_function(title_lo: str) -> str | None:
    ...
    # (c-0) NEW — wrong-function-exec override. A disqualifier fired;
    # if the title is a clearly-wrong-function exec (Finance Director, VP Sales,
    # Public Policy, CISO, Creative Director, etc.), the generic leadership
    # whitelist must NOT rescue it. Kills before (c-i). The bare
    # \bvp\b / \bdirector\b whitelist patterns were exempting these straight
    # into wasted Haiku calls. See WRONG_FUNCTION_EXEC_NEVER_RESCUE.
    if _NEVER_RESCUE_RE.search(title_lo):
        return f"wrong_function_exec:{matched_kw}"
...
def prefilter(job: dict, prefs: dict | None = None) -> dict[str, Any]:
    ...
    is_dream = _is_dream_company(company_normalized, industry_score)

    # POSITIVE relevance gate — the big cost lever.
    # The checks above are a DENY-LIST; everything not explicitly killed fell
    # through to a paid Haiku call, and the audit found 86% of those
    # calls scored <20 (Wedding Coordinator, Ultrasound Technologist, etc.).
    # Skip pre-Haiku ONLY when a job carries NO relevant signal at all: not a
    # known/relevant industry, not a dream company, not a leadership title, and
    # no relevance noun in the TITLE. A real the original author role satisfies >=1 of these
    # (the AI-platform lane is kept via the ai/platform/strategy/engineering
    # nouns even though OpenAI/Anthropic aren't in COMPANY_INDUSTRY_MAP).
    # Conservative by construction: a false-keep costs one cheap call; only a
    # truly signalless title is dropped.
    if (
        not hard_disqualifiers
        and industry == "unknown"
        and not is_dream
        and not is_leadership_title
        and not _RELEVANCE_NOUN_RE.search(title_lo)
    ):
        hard_disqualifiers.append("no_relevant_signal")
        first_reason = first_reason or "no_relevant_signal"
